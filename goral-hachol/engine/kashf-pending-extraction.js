...
// TOPIC_HOUSE_ROLES_KASHF — תפקידים ספציפיים של בתים לפי נושא.
// מקור: כשף אל-אסראר, שער שישי + פסקה כוללת עמ' 166.
// הוצא מ-hawi-interpreter.js (סבב חמישי, 2026-07-07) — ישב שם תחת השם
// TOPIC_HOUSE_ROLES ושימש לחישוב שדות topicRole/specificRolesHebrew, אך אלה
// לא נצרכו בפועל בשום מקום (goral-conclusion-writer.js, UI) — קוד מת. בכל
// זאת, תוכן שמקורו כשף אינו שייך לקובץ חאווי. נשמר כאן כמחסן, לא חובר
// למנוע כשף החי — לא אומת אם ומתי rules אלה ("תפקיד בית X לנושא Y") נצרכות
// כמשפט מפורש בטקסט המקור, או שהן תמצות/פרפרזה של פרק הבתים.
export const TOPIC_HOUSE_ROLES_KASHF = {
...
//
// תוקן 2026-07-07: הטבלה הקודמת כאן לא תאמה את טבלת התמונה האמיתית
// שבמקור (kashf-al-asrar.html, עמ' 199) — כמעט כל הערכים היו שגויים,
// כולל הערת-מיוחד השגויה לנשוא ראש (שערבבה שמות חלופיים של צורות
// אחרות — תשמיר=כבוד נכנס, דגל השמחה=סף יוצא — בטעות). תוקן ישירות
// מול טבלת התמונה. נלחם (1121) אינה מופיעה בטבלה במקור כלל.
const FIGURE_BODY_PART_KASHF = {
  '2121': 'הכתף',           // ממון נכנס
  '1211': 'היד הימנית',     // בר הלחי
  '1111': 'איבר המין',      // דרך
  '2221': 'אחוריים',        // שפל ראש
  '2212': 'הבטן',           // לבן
  '1221': 'החזה',           // סוהר
  '2122': 'הצוואר',         // אדום
  '1222': 'הראש',           // נשוא ראש
  '1112': 'הרגל השמאלית',   // סף יוצא
  '1122': 'הירך השמאלית',   // כבוד יוצא
  '2222': 'הצד השמאלי',     // קהלה
  '1212': 'הכתף השמאלית',   // ממון יוצא
  '2111': 'הרגל',           // סף נכנס
  '2211': 'הירך',           // כבוד נכנס
  '2112': 'הצד הימני',      // חיבור
  // '1121' (נלחם): אינה מופיעה בטבלת המקור כלל — לא הומצא ערך.
};
...
export function computeBodyPartDiagnosisKashf(chart) {
  ...
  if (!bodyPart) return {
    ...
    outputHebrew: `${figHebrew} בבית 6 — האיבר הכואב: לא מופיע בטבלה במקור (כשף עמ׳ 199)`,
  };
  return {
...
// כשף אל-אסרר עמ׳ 225 (H7 instruction), עמ׳ 231-233 (detailed table)
...
export function computeThiefPhysicalDescriptionKashf(chart) {
  ...
  if (!desc) return {
    ...
    outputHebrew: `${figHebrew} בבית 7 — תיאור הגנב: לא מפורש במקור (כשף עמ׳ 231-233)`,
  };
  return {
