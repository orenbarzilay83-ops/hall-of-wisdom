 ...
 *
 * הערה על המספר "16" — עודכן 2026-07-07: הספר מצהיר בפתיחת השער
 * ("שיבוץ... והם שישה־עשר סדרי שיבוץ") ובחתימתו ("הרי אלו שישה־עשר סדרי
 * שיבוץ") שיש 16 סדרים. בקריאה שיטתית של גוף השער עצמו (עמ' 104-150)
 * נמצאו ותועדו כאן 15 סדרים בלבד (שיבוצים 1-15 להלן). ה-16 האפשרי נמצא
 * בסופו של דבר — לא בתוך גוף השער עצמו, אלא **מוזכר בשם מפורש בפרק
 * אחר לגמרי** (עמ' 189, בפרק "האם הדבר במקומו?" בשער השישי, בדין עומק
 * דבר נסתר/מים): "שיבוץ ההפכים" (ראה SHIBUTZ_16_OPPOSITES_DEPTH להלן).
 * ייתכן שזהו ה-16 החסר שהמחבר מונה בפתיחה ובחתימה, אף שלא נכלל בטבלת
 * גוף השער עצמו — לא מוכרע בוודאות, אך זו ההשלמה הקרובה ביותר שנמצאה.
 *
 * קובץ זה עדיין לא מחובר לשום מנוע חי (חוץ מ-SHIBUTZ_1_MOSHAV/
 * SHIBUTZ_2_CANONICAL_NUMBER/SHIBUTZ_3_ELEMENT_VALUES שכבר משמשים ב-
...
//
// תוקן/הושלם (2026-07-07): הגרסה הקודמת כאן קידדה רק 4/16 צורות (מהשיר
// בעמ' 139) וסימנה "לא פוענח בביטחון" לשאר. בפועל קיימת בעמ' 138 עצמה
// "תמונת שיבוץ האותיות" — טבלה מלאה ונקייה (3 שורות: אות ראשית, צורה,
// אות משנית) המכסה את כל 16 הצורות במפורש, ללא צורך בפענוח-שיר. הטבלה
// אומתה: 4 השיוכים המפורשים בשיר (עמ' 139: "ומ״ם לקהלה, נו״ן לסוהר;
// וסי״ן לחיבור, ועי״ן לדרך") תואמים אות-באות לעמודות 1-4 של הטבלה —
// אישוש הדדי בין שני ציטוטים עצמאיים באותו ספר.
//
// הכלל הכללי (עמ' 139: "הצורות המתהפכות מקבלות שתי אותיות, ואילו
// הצורות שאינן מתהפכות מקבלות אות אחת") **אינו הכלל המארגן בפועל**
// (זו הייתה טעות-פענוח בגרסה הקודמת) — הטבלה עצמה מראה של-4 צורות
// בלבד (עמודות 1-4: דרך, חיבור, סוהר, קהלה) אין אות משנית, לכל 12
// הנותרות יש. ארבע אלו אינן "הצורות המתהפכות" (שהן דווקא נלחם/בר-הלחי/
// סוהר/דרך) — הן **ארבע צורות היסוד של שער 5** (ראה
// kashf-gate5-foundational-figures.js::FOUR_FOUNDATION_FIGURES: סוהר,
// חיבור, קהלה, דרך) — אישוש-צולב בלתי-תלוי נוסף בין שער 3 לשער 5.
//
// שים לב: זהו מקור שונה לחלוטין מ-kashf-figure-appearance.js (עמ' 68-95,
// "תיאור הצורות", שיר-שמות אחר עם מטרה שונה — תיאור פיזי/שם אדם, לא
// חשבון-אותיות של שער 3). לפי החלטת המשתמש המפורשת, kashf-figure-
// appearance.js נשאר ללא שינוי; הטבלה כאן היא המקור הרלוונטי לחשבון
// אותיות-לפי-שיבוץ (למשל שער 4 סוג 4, ראה kashf-dhamir.js).
// ─────────────────────────────────────────────────────────────────────────

// תמונת שיבוץ האותיות (עמ' 138) — טבלה מלאה, 16/16 צורות (אות ראשית),
// 12/16 עם אות משנית (4 צורות היסוד — דרך/חיבור/סוהר/קהלה — ללא אות
// משנית, לפי מבנה הטבלה המקורי עצמו, לא חוסר-פענוח).
export const SHIBUTZ_6_LETTER_TABLE = [
  { pattern: '1111', hebrewName: 'דרך',        primaryLetter: 'ע', secondaryLetter: null },
  { pattern: '2112', hebrewName: 'חיבור',       primaryLetter: 'ס', secondaryLetter: null },
  { pattern: '1221', hebrewName: 'סוהר',        primaryLetter: 'נ', secondaryLetter: null },
  { pattern: '2222', hebrewName: 'קהלה',        primaryLetter: 'מ', secondaryLetter: null },
  { pattern: '1212', hebrewName: 'ממון יוצא',   primaryLetter: 'ל', secondaryLetter: "ע'" },
  { pattern: '2121', hebrewName: 'ממון נכנס',   primaryLetter: 'כ', secondaryLetter: "ט'" },
  { pattern: '1211', hebrewName: 'בר הלחי',     primaryLetter: 'י', secondaryLetter: "צ'" },
  { pattern: '1121', hebrewName: 'נלחם',        primaryLetter: 'ט', secondaryLetter: 'ד' },
  { pattern: '1112', hebrewName: 'סף יוצא',     primaryLetter: 'ח', secondaryLetter: "ח'" },
  { pattern: '2111', hebrewName: 'סף נכנס',     primaryLetter: 'ז', secondaryLetter: "ת'" },
  { pattern: '2211', hebrewName: 'כבוד נכנס',   primaryLetter: 'ו', secondaryLetter: 'ת' },
  { pattern: '1122', hebrewName: 'כבוד יוצא',   primaryLetter: 'ה', secondaryLetter: 'ש' },
  { pattern: '2212', hebrewName: 'לבן',         primaryLetter: 'ד', secondaryLetter: 'ר' },
  { pattern: '2122', hebrewName: 'אדום',        primaryLetter: "ג'", secondaryLetter: 'ק' },
  { pattern: '2221', hebrewName: 'שפל ראש',     primaryLetter: 'ב', secondaryLetter: 'צ' },
  { pattern: '1222', hebrewName: 'נשוא ראש',    primaryLetter: 'א', secondaryLetter: 'פ' },
];
export const SHIBUTZ_6_LETTER_TABLE_META = {
  sourceStatus: 'explicit-in-source',
...
// נשמר לתאימות/תיעוד — תת-קבוצה של SHIBUTZ_6_LETTER_TABLE (4 השיוכים
// שהשיר בעמ' 139 מוסר במפורש, לאישוש-צולב מול הטבלה).
...
// ─────────────────────────────────────────────────────────────────────────
// שיבוץ (אפשרי) 16: שיבוץ ההפכים — עומק דבר נסתר/מים (עמ' 189, בשער
// השישי, לא בגוף השער השלישי — ראה הערה בראש הקובץ).
//
// המקור: "אם רצית לדעת את עומק המים או הדבר הטמון באדמה: דע כי האש
// נמדדת באצבע, האוויר בשיבר, המים באמה, והעפר לפי עומק הקרקע והדבר
// הקבור... התבונן מה יש בה מארבעת היסודות, וקח לכל יסוד את שיעור
// אורכו... סדר זה ידוע בשם שיבוץ ההפכים."
//
// חשוב: זו טבלת אורך-יסוד שונה לגמרי מ-SHIBUTZ_3_ELEMENT_CORRESPONDENCES
// (עמ' 123: אש=שיבר, אוויר=אמה, מים=קומה, עפר=פישוק ידיים) — אין לערבב
// ביניהן. שיטת החישוב אומתה מול שתי הדוגמאות המפורשות במקור: לוקחים
// את **כל** שורות היסוד הפתוחות בצורה (לא רק היסוד הדומיננטי), ומצרפים
// את מידת-האורך של כל אחת. "לפי היסוד השולט דון על עומק הדבר" (המשפט
// הראשון) קובע את אופי/סוג המקום; חישוב האורך עצמו הוא לפי כל היסודות
// הפתוחים, כפי שמאשרות שתי הדוגמאות:
//   נשוא ראש (1222, רק אש פתוחה) → אצבע בלבד.
//   כבוד יוצא (1122, אש+אוויר פתוחות) → "אצבע וטפח" (אצבע + מידת האוויר;
//     המקור עצמו נוקט כאן "טפח" ולא "שיבר" כפי שנאמר בכלל הכללי לעיל —
//     חוסר-עקביות מינוח קל במקור עצמו, לא תוקן/הוכרע כאן).
// ─────────────────────────────────────────────────────────────────────────

export const SHIBUTZ_16_OPPOSITES_DEPTH_MEASURES = {
  'אש':   'אצבע',
  'אוויר':'שיבר / טפח',
  'מים':  'אמה',
  'עפר':  'עומק הקרקע / קומת אדם',
  sourceStatus: 'explicit-in-source',
  sourceRef: "כשף אל-אסראר עמ' 189",
};

/**
 * מחזיר את רשימת מידות-האורך (אחת לכל יסוד פתוח בצורה, לפי סדר
 * הצורה אש/אוויר/מים/עפר) לפי שיבוץ ההפכים.
 * @param {string} pattern — e.g. '1122'
 * @returns {string[]}
 */
export function computeHiddenDepthByOpposites(pattern) {
  const ELEMENT_ORDER = ['אש', 'אוויר', 'מים', 'עפר'];
  return pattern
    .split('')
    .map((digit, i) => (digit === '1' ? ELEMENT_ORDER[i] : null))
    .filter(Boolean)
    .map((el) => SHIBUTZ_16_OPPOSITES_DEPTH_MEASURES[el]);
}

// ─────────────────────────────────────────────────────────────────────────
// "המבקש במעגל" (עמ' 218-219, בשער השישי, פרק יוקר/זול/קנייה/מכירה) —
// שיטה נפרדת המשתמשת ב"בית כבוד" של הצורה שבבית הראשון, המבוססת על
// "מעגל נציר א-דין" (אותו מקור המוזכר גם בכותרת עמ' 98 עבור טור
// "המעלה" ב-FIGURE_DIGNITIES).
//
// חשוב: "בית כבוד" כאן **אינו** מוכח כזהה ל-FIGURE_DIGNITIES.maalaHouse
// (עמ' 96-99) — מתוך 2 הדוגמאות המפורשות של הכלל הזה (עמ' 219), רק אחת
// (שפל ראש=8) תואמת את טבלת המעלה; השנייה (כבוד נכנס=12) סותרת אותה.
//
// הערה חשובה על עמ' 220: יש שם דוגמה נוספת (ממון יוצא, "בית כבודו
// הרביעי") — אך היא שייכת לכלל **אחר** ("דיני המבוקש במעגל", לא
// "המבקש במעגל"): שם סופרים "מבית כבודו עד המקום שבו נמצא" (כלומר עד
// מקום ההימצאות בפועל בלוח), לא "עד הבית הראשון" כמו בכלל המבקש. בנוסף,
// החשבון בדוגמה עצמה אינו מתיישב באף פירוש שבדקתי (מ-4 עד 4 אינו נותן
// 7). מפני שזה כלל שונה במפורש, ומפני שהחשבון בו אינו מאומת, הערך
// ממון-יוצא=4 **לא נכלל כאן** — לא הוחל בטעות על כלל המבקש.
// ─────────────────────────────────────────────────────────────────────────

export const REQUEST_CIRCLE_HONOR_HOUSES = {
  '2221': 8,  // שפל ראש — "בית כבודה השמיני" (עמ' 219)
  '2211': 12, // כבוד נכנס — "בית כבודה השנים־עשר" (עמ' 219)
  sourceStatus: 'explicit-in-source',
  sourceRef: "כשף אל-אסראר עמ' 218-219",
  note: 'רק 2 מתוך 16 הצורות מאומתות במפורש עבור כלל "המבקש במעגל" הזה. אין להשלים את שאר 14 הצורות בהיקש/ניחוש, ואין לערבב עם דוגמת ממון-יוצא מעמ׳ 220 (כלל שונה — "המבוקש במעגל").',
};

/**
 * "המבקש במעגל" (עמ' 218-219): לצורה שבבית הראשון, סופרים מ"בית כבודה"
 * עד הבית הראשון (מעגלי, קדימה, כולל שני הקצוות), ואז הולכים את אותו
 * מספר צעדים מהבית הראשון (בסדר הבתים הרגיל) — הבית שמגיעים אליו מורה
 * על כוח המבקש/הבקשה.
 * מחזיר null אם הצורה שבבית הראשון אינה אחת מ-2 הצורות המאומתות.
 */
export function computeRequesterCircleHouse(firstHousePattern) {
  const honorHouse = REQUEST_CIRCLE_HONOR_HOUSES[firstHousePattern];
  if (!honorHouse) return null;
  ...
  const landingHouse = ((count - 1) % 16) + 1;
  return { honorHouse, count, landingHouse };
}
