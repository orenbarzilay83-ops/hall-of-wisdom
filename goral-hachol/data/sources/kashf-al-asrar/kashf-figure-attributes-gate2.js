 ...
 * שנגזר בזמן טעינת kashf-dhamir.js::FIGURE_MAALA_HOUSE לצורך סוג 2
...
//
// ממון נכנס (2121) וסוהר (1221) אינם מקבלים שורה בטבלה המודפסת — לא
// הומצא עבורם ערך. ערכי null מציינים "לא נתפרש במקור" או ניסוח יחסי
// שאסור להמיר לבית מספרי בלי כלל מקור נוסף.
// ─────────────────────────────────────────────────────────────────────────
...
export const FIGURE_DIGNITIES = {
  ...
  '1212': { // ממון יוצא
    ...
    note: 'צערה נאמר במקור כ"כנגדו"; אין להמיר את הניסוח היחסי לבית מספרי בלי כלל מקור נוסף.',
  },
  '2222': { // קהלה
    maalaHouse: 16, moshavHouse: 2, gvulHouse: 11, panimHouse: 10,
    simchaHouse: 10, tzaarHouse: null, mezegHouse: 9,
  },
  // '2121' (ממון נכנס) ו-'1221' (סוהר) — אינן מקבלות שורה בטבלה המודפסת.
};

// ─────────────────────────────────────────────────────────────────────────
// פרק נוסף בשמחת/צער הצורות (עמ' 100) — הערות משלימות, נפרדות מהטבלה
// לעיל, המקשרות שמחת-צורה לכוכב אחר ולא לבית:
// "שמחת צורות שבתאי נמצאת בשמחת צורות צדק — בבית האחד-עשר. שמחת צורות
//  מאדים בשישה-עשר; שמחת צורות השמש בתשיעי; שמחת צורות נוגה בחמישי;
//  שמחת צורות כוכב חמה
...
//  שמחתן באחד-עשר. קהלה וחיבור — צערן בשביעי ובארבעה-עשר. לבן ודרך —
//  צערן בראשון." ממון יוצא וסף יוצא נזכרים יחד בניסוח מסויג
//  כנספחים אפשריים לשבתאי ולמאדים.
// ─────────────────────────────────────────────────────────────────────────
export const FIGURE_JOY_GRIEF_SUPPLEMENTARY_NOTE = {
  ...
  griefByFigure: {
    ...
    '2212': [1],      // לבן — צערו בראשון
    '1111': [1],      // דרך — צערו בראשון
  },
  tentativePlanetAttachments: {
    status: 'source-tentative',
    figures: ['1212', '1112'], // ממון יוצא, סף יוצא
    planetsHebrew: ['שבתאי', 'מאדים'],
    noteHebrew: 'המקור אומר על שתי הצורות יחד כי אולי הן נספחות לשבתאי ולמאדים; אין להפוך זאת לשיוך ודאי.',
  },
};

// ─────────────────────────────────────────────────────────────────────────
// שיוך הצורות לחודשים (עמ' 100-101, "פרק בשיוך הצורות לחודשים")
// חודשי הלוח ההיג'רי. חלק מהחודשים משויכים ליותר מצורה אחת, וצורת בר
// הלחי אינה משויכת לחודש בפרק. אין לכפות יחס של אחד-לאחד.
// ─────────────────────────────────────────────────────────────────────────
export const FIGURE_MONTHS = {
  sourceStatus: 'explicit-in-source',
  sourceRef: "כשף אל-אסראר עמ' 100-101",
  provenance: {
    workId: 'kashf-al-asrar',
    editionId: 'kashf-hebrew-v56',
    sourceLayer: 'author_main_text',
  ...
  byMonth: {
    ...
    'שוואל':          ['2222', '2212'], // קהלה, לבן
    'רביע הראשון':     ['1121'],         // נלחם
    'רביע האחרון':     ['1121', '2111'], // נלחם, סף נכנס
  ...
  unassignedFigures: ['1211'], // בר הלחי
};
