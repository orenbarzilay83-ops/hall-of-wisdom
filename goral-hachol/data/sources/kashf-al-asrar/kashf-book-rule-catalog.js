...
export const KASHF_BOOK_RULE_CATALOG = [
  ...
  },
  {
    ...
    enginePath: null,
    confidence: 'verified',
    unresolvedReason: 'Printed pp101-102 explicitly use H5 as the third witness (H5→[3,7,11]), not H15. Testimony targets for H14 differ from the page-53 scheme ([2,6,10] vs [5,6,11]); H9/H5/H16 are witnesses here under this extended scheme. No reconciling source text was found between the p53 and p101-102 schemes. Do NOT merge or choose between the two schemes without an explicit source rule. This entry has no runtime evidence whatsoever — it can never appear in appliedBookRules.',
    sourceVerificationStatus: 'verified-exact-quote',
    applicabilityStatus: 'unresolved',
    resolutionStatus: 'unresolvedSourceRelationship',
    runtimeEvidence: {
  ...
  },
  {
    ...
    appliesToTopics: ['spiritualDiagnostics'],
    appliesToQuestionTypes: ['spiritual'],
    triggerConditions: ['explicit runtime opt-in only; not default reading execution'],
    requiredHouses: null,
    requiredFigures: null,
    calculationType: 'majority vote across the 5 dhamir types ("ותכריע לפי הרוב")',
    verdictEffect: 'reference/advisor-only aggregation rule; preserved from source but disconnected from default runtime',
    precedence: null,
    conflictHandling: null,
    ...
    runtimeEvidence: {
      ...
      feedsOverallPositive: false,
      feedsNarrativeOrProfessionalFinding: false,
      evidenceNote: 'The p155 majority rule remains implemented as computeDhamirByMajority() for source fidelity, but buildKashfReading() no longer calls it automatically. Runtime Dhamir now requires an approved hiddenThoughtIntent plus one explicit method selection; therefore this aggregation rule is available code/reference, not an applied finding by default.',
    },
  },
];
 ...
 * being "topic-independent" in the source does NOT make any sub-method
 * automatically runnable on every reading. Implemented methods are now
 * availability facts only; runtime evaluation requires an approved Dhamir
 * intent and one explicit selected method. The p155 majority rule remains
 * preserved as source knowledge but is disconnected from default runtime.
 *
...
export const KASHF_DHAMIR_METHOD_COVERAGE = [
  {
    ...
    sourcePage: '151-152',
    triggerCondition: 'זמין בקוד; runtime דורש intent מאושר ובחירה תפעולית מפורשת',
    ...
    runtimeEvidence: {
      ...
      feedsOverallPositive: false,
      feedsNarrativeOrProfessionalFinding: false,
      evidenceNote: 'The method remains implemented but is not evaluated on ordinary readings. It can enter reading.dhamir only through an explicit approved need-driven Dhamir selection; implementation availability alone is not runtime evidence.',
    },
  },
  {
  ...
  },
  {
    ...
    runtimeEvidence: {
      ...
      feedsOverallPositive: false,
      feedsNarrativeOrProfessionalFinding: false,
      evidenceNote: 'The external Type-4 helper remains implemented but is no longer computed by default; it requires enableExternalDhamirType4===true and remains external/advisor-only.',
    },
  },
  {
