...
export const KASHF_TOPICS_PER_HOUSE = {
  ...
  2: {
    ...
    topics: [
      ...
      { label: 'פרנסה ומחיה',             description: 'מצב הפרנסה, מאיפה תבוא הפרנסה — PDF1: المكاسب، مال الأب، ما يملكه أو يقبضه',                    implemented: true,  topicId: 'commerce',              sourceNote: 'multi-source — PDF1 p.24, kashf p.181-182' },
      { label: 'מקור הכסף',               description: 'מאיפה מגיע הכסף: שלטון / נשים / מסחר / ירושה / ספרים — לפי כוכב בעל הבית',                      implemented: true,  topicId: 'commerce',                sourceNote: 'source: kashf' },
      { label: 'מטמון / כנוז תת-קרקעי',  description: 'PDF1 p.24: والكنوز والدفاين — בית 2 מורה גם על מטמונות. שיטה: בית 2+6 יחד עם בית 4',             implemented: true,  topicId: 'hiddenTreasure',        sourceNote: 'source: PDF1 p.24' },
    ],
  },
  ...
  4: {
    ...
    topics: [
      ...
      { label: 'נסתרות / סודות',          description: 'PDF1 p.24: الخبايا والأسرار القديمة — דבר חבוי שלא ידוע; גם: כישוף קבור (מוזכר בבית 4 ב-PDF1)',    implemented: true,  topicId: 'hiddenTreasure',        sourceNote: 'source: PDF1 p.24' },
      { label: 'סיבת המחלה',              description: 'PDF1 p.24: بيت سبب المرض — בית 4 הוא "בית סיבת המחלה"; שיטה: השווה ב4 עם ב6 לבחינת מקור',         implemented: true,  topicId: 'illness',        sourceNote: 'source: PDF1 p.24' },
    ],
  },
  ...
  5: {
    ...
    topics: [
      ...
      { label: 'זכר או נקבה',             description: 'כשף עמ׳ 193: קח מב1+ב5 → אש/אוויר=זכר, מים/עפר=נקבה. גם ב11: כשסעיד יוצא ב5+11=זכר, סעיד נכנס=נקבה. [ממומש ב-childrenPregnancyKashf]',  implemented: true, topicId: 'childrenPregnancy', sourceNote: 'source: kashf p.193' },
      { label: 'חשש להפלה',              description: 'כשף עמ׳ 191-192: אדום/שפל-ראש בב7 = סכנת הפלה; אדום בב8 = הפלה; סוהר בב8 = סכנה לאם ולנולד. [ממומש ב-childrenPregnancyKashf]',  implemented: true, topicId: 'childrenPregnancy', sourceNote: 'source: kashf p.191-192' },
      { label: 'כמה חודשים הריון',        description: 'כשף עמ׳ 192: הצורה במאזן (בית 15) = מספר חודשי ההריון — ספירת הנקודות. [ממומש ב-childrenPregnancyKashf]',              implemented: true, topicId: 'childrenPregnancy', sourceNote: 'source: kashf p.192' },
      { label: 'לידה קלה או כבדה',       description: 'כשף עמ׳ 191: בית 5 ת׳אבת = לידה קשה; מנקלב = לידה קלה. [ממומש ב-childrenPregnancyKashf]',                                         implemented: true, topicId: 'childrenPregnancy', sourceNote: 'source: kashf p.191' },
      { label: 'מצב הילדים',             description: 'כשף עמ׳ 194: ב6 מזיק = מחלות בגיל הרך; ב8 מיטיב = ישתפר. [ממומש ב-childrenPregnancyKashf]',      implemented: true, topicId: 'childrenPregnancy', sourceNote: 'source: kashf p.193-194' },
      { label: 'אורך חיים של הילד',      description: 'כשף עמ׳ 194: ב5+16 שניהם מיטיב = אושר ועושר; שניהם מזיק = ירוד; מעורב = בינוני. [ממומש ב-childrenPregnancyKashf]',                  implemented: true, topicId: 'childrenPregnancy', sourceNote: 'source: kashf p.194' },
      { label: 'שמחות ואירועים',          description: 'PDF1 p.24: الأفراح والطعام والشراب والأخبار والرسائل والولائم والحفلات — שמחות, אירועים, ביגוד',  implemented: true,  topicId: 'children',         sourceNote: 'source: PDF1 p.24' },
    ],
  },
  ...
  7: {
    ...
    topics: [
      ...
      { label: 'סכסוך / תביעה',          description: 'מי ינצח — גובר ונגבר. PDF1 ch.16: الفصل السادس عشر الغالب والمغلوب. PDF2 ch.8: בתים 1+2+7+8; שופט=ב10; מגשרים=ב3+4+13+14',            implemented: true,  topicId: 'disputes',     sourceNote: 'multi-source — PDF1 ch.16, PDF2 ch.8, kashf' },
  ...
  8: {
    ...
    topics: [
      { label: 'גנבה',                   description: 'PDF1 p.47: ב1=נגנב, ב2=כמות, ב3=מספר, ב4=מאיפה נגנב, ב6=סוג הגנבה, ב7=גנב, ב8=כסף הגנב, ב10=מיקום הגנב. אם לא מצא חמרה/נקי/ממון יוצא/סף יוצא = לא נגנב',  implemented: true,  topicId: 'theft',             sourceNote: 'multi-source — PDF1 p.47-49, kashf' },
      { label: 'תיאור פיזי של הגנב',     description: 'כשף עמ׳ 231-234: ב7 = תיאור הגנב — כל 16 הצורות ממופות לתיאור פיזי ומקצועי (THIEF_PHYSICAL_DESCRIPTION_KASHF)',                                                                    implemented: true,  topicId: 'theft',             sourceNote: 'source: kashf p.231-234' },
      { label: 'קרבת הגנב לשואל',        description: 'כשף עמ׳ 224: ב7 חוזר בב1=עומד במקום בעל הדבר; ב2=מן העוזרים; ב3=זה הגנב; ב4=נכנס לביתו; ב5=מתערב עם ילדיו; ב6=יחלה; ב7=גילוי ודאי; ב9=בגלל נסיעה; ב10=קשור לשלטון; ב11=מכרי הגנב', implemented: true,  topicId: 'theft',             sourceNote: 'source: kashf p.224' },
      ...
      { label: 'ביטחון ואורך חיים',      description: 'מיטיב בבית 8 = ביטחון; מזיק = פחד, מוות פתאומי',     implemented: true,  topicId: 'deathInheritance', sourceNote: 'source: kashf p.234-236' },
    ],
  },
  ...
  9: {
    ...
    topics: [
      ...
      { label: 'כיוון הנסיעה',           description: 'PDF1 ch.10: יסוד צורה בב9 → נארי=דרום, תראבי=מזרח, מאאי=צפון, הוואאי=מערב',  implemented: true,  topicId: 'travel',        sourceNote: 'source: PDF1 ch.10' },
      { label: 'בחירת זמן לנסיעה',      description: 'מתי לצאת — לפי בית 9 וכוחות בלוח',                 implemented: true,  topicId: 'travel',            sourceNote: 'source: kashf' },
      { label: 'נעדר — חי או מת',       description: 'PDF1 p.52 + PDF2 p.59: הסתכל ב1+ב4+ב8+ב9+ב12. ב6+ב7+ב9+ב10 לחיות. ג׳ודלה+חמרה+ג׳מאעה+סף יוצא בב4+ב12+ב14 = מת. מיטיב בב10 = חי',  implemented: true,  topicId: 'missingPerson', sourceNote: 'multi-source — PDF1 p.52, PDF2 p.59, kashf' },
      { label: 'מיקום הנעדר',            description: 'כשף עמ׳ 246-250: ב7=בית הנעדר; ב9+10 = כיוון; ב5 מיטיב = קרוב לבית; ב6 ממון-יוצא = בגלות. [ממומש ב-missingPersonLocation]',  implemented: true, topicId: 'missingPerson',    sourceNote: 'source: kashf p.246-250' },
      { label: 'האם הנעדר יחזור',        description: 'כשף עמ׳ 246-251: ב14 נשוא-ראש/כבוד-נכנס=יחזור; ב1+ב7 מיטיב=סיכוי טוב; ב6 ממון-יוצא=סכנת מוות בגלות. [ממומש ב-missingPersonReturn]',      implemented: true, topicId: 'missingPerson',    sourceNote: 'source: kashf p.246-251' },
      { label: 'חלום',                   description: 'PDF1 p.24 (בית 9): والمنامات وطلب العلم. גם בית 3 (PDF1 p.24). בית 9 = חלומות דתיים ורוחניים; בית 3 = חלומות רגילים קצרי-טווח',           implemented: true,  topicId: 'travel',             sourceNote: 'multi-source — PDF1 p.24, kashf p.254' },
      ...
      { label: 'כישוף / מכשפים',         description: 'PDF1 p.56: والبيت التاسع يدل على الساحر والشيوخ الروحانيين والذين يعملون في أمور السحر والجن والعرافة والكهانة والشعوذة',                    implemented: true,  topicId: 'spiritualDiagnostics', sourceNote: 'source: PDF1 p.56' },
      { label: 'מקצוע / עיסוק',         description: 'לפי כוכב בית 9 — חקלאות, מדעים, רפואה, אמנות',    implemented: true,  topicId: 'foundations',            sourceNote: 'source: kashf' },
      { label: 'הבטחה — האם תתממש',     description: 'מסאלה: האם ייקיים הבטחה שנתן',                      implemented: true,  topicId: 'completion',            sourceNote: 'source: kashf' },
    ],
  },
  ...
  10: {
    ...
    topics: [
      ...
      { label: 'האם יחזור לתפקיד',      description: 'מסולק שרוצה לחזור — בית 1+10+11 נכנס',           implemented: true,  topicId: 'authorityState',  sourceNote: 'source: kashf p.266' },
  ...
  11: {
    ...
    topics: [
      ...
      { label: 'תקווה / אמל',            description: 'האם מה שמקווים לו יתממש — PDF1 p.51: اضرب 1×5 — נאתג׳ נכנס = יתמלא',                                implemented: true,  topicId: 'completion', sourceNote: 'multi-source' },
      ...
      { label: 'האם ישיג מה שרוצה',      description: 'PDF1 p.51 ch.12: قضاء الحاجة — לפי ב1+ב11+ב15 נכנסים. PDF1 p.52: ב11 = בית הרג׳אאה',               implemented: true,  topicId: 'completion',    sourceNote: 'multi-source — PDF1 p.51-52, kashf' },
  ...
  12: {
    ...
    topics: [
      ...
      { label: 'חובות',                  description: 'PDF1 p.24: بيت الديون والأسر والاعتقال — בית 12 = חובות וכפייה',                                                  implemented: true,  topicId: 'loan',                 sourceNote: 'source: PDF1 p.24' },
    ],
  },
