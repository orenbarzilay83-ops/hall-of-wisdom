...
export const KASHF_TOPICS_PER_HOUSE = {
  ...
  5: {
    ...
    topics: [
      ...
      { label: 'זכר או נקבה',             description: 'כשף עמ׳ 193: קח מב1+ב5 → אש/אוויר=זכר, מים/עפר=נקבה. גם ב11: כשסעיד ח׳ארג ב5+11=זכר, סעיד דאח׳ל=נקבה. [ממומש ב-childrenPregnancyKashf]',  implemented: true, topicId: 'childrenPregnancy', sourceNote: 'source: kashf p.193' },
  ...
  7: {
    ...
    topics: [
      ...
      { label: 'ת׳יב / בכר (האישה)',     description: 'PDF1 p.44: لمعرفة المرأة ثيب أم بكر — כפל 13×7; פתוחה=ת׳יב, מסגורת=בכר. PDF2 p.44: הסתכל בבית 11 (קבץ דאח׳ל) ותוצאה מציינת',               implemented: true,  topicId: 'marriage',             sourceNote: 'multi-source — PDF1 p.44, PDF2 p.44' },
  ...
  9: {
    ...
    topics: [
      ...
      { label: 'נעדר — חי או מת',       description: 'PDF1 p.52 + PDF2 p.59: הסתכל ב1+ב4+ב8+ב9+ב12. ב6+ב7+ב9+ב10 לחיות. ג׳ודלה+חמרה+ג׳מאעה+עתבה ח׳ארגה בב4+ב12+ב14 = מת. מיטיב בב10 = חי',  implemented: true,  topicId: 'missingPerson', sourceNote: 'multi-source — PDF1 p.52, PDF2 p.59, kashf' },
  ...
  12: {
    ...
    topics: [
      ...
      { label: 'פרטי שחרור / עונש',      description: 'PDF1 p.54-55: מות בכלא: חמרה+ג׳מאעה חוזרות בב4+ב8+ב9 = מות שם. עתבה ח׳ארגה בב1+ב3 ועתבה+קבץ ח׳ ב8+ב12 = בורח. אנכיס בב1 = חולה בכלא. ג׳ודלה בב1+ב12 = עדיין כלוא',  implemented: true,  topicId: 'prisoner',             sourceNote: 'source: PDF1 p.53-55' },
      { label: 'כלא — מי אשם (מין)',      description: 'PDF1 p.54: המסבב לכלא — כפל 7×1 לגנוסו. עתבה ח׳ארגה בב1+ב3 = בורח. ג׳ודלה בב1+ב12 = עצור עדיין',             implemented: true,  topicId: 'prisoner',             sourceNote: 'source: PDF1 p.54' },
