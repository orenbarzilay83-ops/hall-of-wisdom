...
export const KASHF_CANONICAL_METHODS = Object.freeze({
  ...
  'spiritual.p167.hiddenActionAirRows46815': method({
    ...
    executionKind: 'custom-engine',
    executorStatus: 'ready',
    notes: 'Canonical p167 hidden-action executor is wired. Take ONLY the AIR row of H4, H6, H8 and H15 (the balance/judge) and assemble one four-row figure. The printed question is personal: هل ورائي عمل — whether an action is behind the querent. Pure malefic => an action is behind the querent; every other source classification follows the explicit source complement “otherwise no”. This is covert/hidden action only and must never be promoted to sorcery, jinn, evil-eye or sorcerer identification.',
  }),

  // ── HOUSE 1 p166-170 intent boundaries ------------------------------
  'connection.p166-167.initiatorAndDegree': method({
    kashfMethodId: 'connection.p166-167.initiatorAndDegree',
    kashfIntentId: 'connection.specificPerson.initiatorAndDegree',
    topicId: 'generalReading',
    sourcePages: [166, 167],
    methodRole: 'unresolved',
    kashfRuntimeStatus: 'ready',
    ...
    executionKind: 'custom-engine',
    executorStatus: 'pending',
    notes: 'Source-closed connection method for one specific person: water rows H4/H7/H13/H14, then combine with the balance; external/internal identifies who initiates movement, and element openness separately grades the connection. It is not a spiritual-diagnostics supporting check and has no approved public question route yet.',
  }),

  'need.p168-169.locationNameColorBalanceWalk': method({
    kashfMethodId: 'need.p168-169.locationNameColorBalanceWalk',
    kashfIntentId: 'need.locationNameColor',
    topicId: 'generalReading',
    sourcePages: [168, 169],
    methodRole: 'unresolved',
    kashfRuntimeStatus: 'blocked-by-source',
  ...
  'decision.external.p170-172.istikharaFigureTable': method({
    kashfMethodId: 'decision.external.p170-172.istikharaFigureTable',
    kashfIntentId: 'decision.istikharaByFigure',
    topicId: 'generalReading',
    sourcePages: [170, 171, 172],
    sourceLayer: 'quoted-alternate',
    attributedSourceBook: 'other-book',
    methodRole: 'external-tradition',
    kashfRuntimeStatus: 'educational-only',
    runtimeAllowed: false,
    executionKind: null,
    executorStatus: 'not-applicable',
    notes: 'The book explicitly marks this istikhara table as خارج الكتاب / from outside the book. It remains searchable reference knowledge only. It must never auto-route, run in parallel with the selected Kashf body method, or vote against it.',
  }),
